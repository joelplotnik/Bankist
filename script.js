...
const account2 = {
  ...
  movementsDates: [
    ...
    '2020-07-20T14:43:26.374Z',
    '2022-07-23T18:49:59.371Z',
    '2022-07-24T12:01:20.894Z',
  ],
  currency: 'EUR',
  locale: 'pt-PT',
};
...
const updateUI = function (acc) {
  displayMovements(acc); // dispaly movements
...
btnLogin.addEventListener('click', function (e) {
  ...
  // optional chaining (?) only returns a value if it exists (avoids error)
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    const options = {
      ...
      minute: 'numeric',
      day: 'numeric',
      month: 'numeric', // can do numeric, long, or 2-digit as well
      year: 'numeric',
      //weekday: 'long',
    };

    // adding the options allows for time
    labelDate.textContent = new Intl.DateTimeFormat(
      currentAccount.locale,
      options
    ).format(now);

    //// This was the previous way date was displayed
    // const day = `${now.getDate()}`.padStart(2, 0);
    // const month = `${now.getMonth() + 1}`.padStart(2, 0);
    // const year = now.getFullYear();
    // const hour = `${now.getHours()}`.padStart(2, 0);
    // const min = `${now.getMinutes()}`.padStart(2, 0);
    // labelDate.textContent = `${day}/${month}/${year}, ${hour}:${min}`; // time is static

    // clear input fields
    inputLoginUsername.value = inputLoginPin.value = '';
    inputLoginPin.blur(); /// makes it so the field loses it's focus
...
btnTransfer.addEventListener('click', function (e) {
  ...
  if (
    ...
    receiverAcc.movementsDates.push(new Date().toISOString()); // add transfer date
...
btnSort.addEventListener('click', function (e) {
  e.preventDefault();
  displayMovements(currentAccount.movements, !sorted); // the not operator sorts if false
